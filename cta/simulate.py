...
def load_asset(
    ...
    """
    Load an asset, set it as the active context, and return it.
    ...
    Call this before building signals with Prices() / Returns() / etc.
        ...
        asset_obj = cta.load_asset('mtx', '1d')
...
def _load_asset(asset: str, time_granularity: str, contract: str = "front") -> BaseAsset:
    if asset == "mtx" and time_granularity == "1d":
        import csv
        ...
        # Sort by date then expiry ascending; rank 0 = 近月, rank 1 = 次月
        ...
        symbol = "MTX" if contract == "front" else "MTX_back"
        return BaseAsset.from_df(series, symbol=symbol, time_granularity="1d")

    raise NotImplementedError(
        f"Asset '{asset}' with granularity '{time_granularity}' is not yet supported."
    )
...
def simulate(
    ...
    cum_pnl_net = pnl_net.cumsum().rename("cum_pnl_net")
...
def _plot_cum_pnl(ax, pnl: pd.Series, cum_pnl: pd.Series) -> None:
    ...
    sharpe = _sharpe(pnl)
    ax.plot(cum_pnl.index, cum_pnl.values, linewidth=1.0, color="#4fc3f7")
...
def _plot_price_comparison(ax, asset_obj, exec_sig: pd.Series) -> None:
    ...
    series = {
        "close":  (exec_sig * close_ret).cumsum(),
        "open":   (exec_sig * open_ret).cumsum(),
        "vwap≈":  (exec_sig * vwap_ret).cumsum(),
    }
    colors = {
        "close":  "#4fc3f7",
        "open":   "#ffb74d",
    ...
    for name, cum in series.items():
        ax.plot(cum.index, cum.values, linewidth=0.9,
...
def _bar_colors(vals):
    return ["#26a69a" if v >= 0 else "#ef5350" for v in vals]
...
def _plot_front_vs_back(ax, asset_obj, asset_obj_back, exec_sig, pnl, cum_pnl) -> None:
    ...
    if asset_obj_back is not None:
        ...
        cum_back  = pnl_back.cumsum()
        sharpe_back = _sharpe(pnl_back)
        ax.plot(cum_back.index, cum_back.values, linewidth=1.0, color="#ffb74d",
...
def _plot_casr(ax, pnl: pd.Series, trading_index: pd.DatetimeIndex,
               window: int = 30) -> None:
    """(2,1) CASR — delegates to cta.Caar and plots the result."""
    try:
    ...
    ax.plot(x, casr, linewidth=1.2, color="#4fc3f7",
...
def _plot_cost_summary(ax, pnl, cum_pnl, pnl_net, cum_pnl_net, tcost) -> None:
    ...
    ax.plot(cum_pnl_net.index, cum_pnl_net.values, linewidth=1.0, color="#26a69a",
...
def SimulateAll(
    ...
    *sigs            : one or more raw signal Series
    ...
    for i, sig in enumerate(sigs):
        ...
        pnl      = (exec_sig * ret).rename("pnl")
        cum_pnl  = pnl.cumsum()
        turnover = exec_sig.diff().abs().rolling(20, min_periods=1).mean()

        # ── left: cumulative PnL ─────────────────────────────────────────────
        sharpe = _sharpe(pnl)
        ax_pnl.plot(cum_pnl.index, cum_pnl.values, linewidth=1.0, color="#4fc3f7")
