...
**Alpha vs buy-and-hold**
  beta, mean_abs_w, beta_per_w, alpha_ann_pct, corr, info_ratio
  `beta_per_w = beta / mean|exec_w|` is the scale-invariant one (QNT-100);
  plain `beta` can be shrunk under any threshold by cutting exposure. Both are
  REPORTED, not gated: the house gate is `|beta| < 0.15` (QNT-100 rejected,
  2026-09-02), so quote `beta_per_w` beside any sparse survivor yourself.
...
# Below this mean |exec_w| the book is effectively flat and `beta_per_w` is a
# ratio of two numbers that are both noise; report NaN rather than a huge number.
_MIN_ABS_W      = 0.01
...
def signal_stats(
    ...
    auto_flip:      bool  = True,
    roll_adjusted:  bool  = True,     # QNT-21: match the runner / _base.py legs
) -> dict:
    ...
    roll_adjusted : which return series to score against.
        True (default since QNT-21) = `asset.returns`, which prices the roll
        against yesterday's BACK-month close - the same definition the runner's
        `_c2c_ret` and `cta.Simulate` now use, so all three paths agree.
        False = `close.pct_change()`, which on the 305 rollover days
        ...
        was flipped to True on 2026-09-01 (QNT-21, approved by Reggie) together
        with the roll fix to the c2c/o2o/noonpause legs in signals/_base.py.
        Published gate numbers computed before that date were on the raw series
        and are not comparable. See QNT-13, QNT-21.
    ...
    # ── Exposure-normalised beta (QNT-100) ─────────────────────────────────
    # `beta` scales with how much of the time the book is in the market, so a
    # long-only mask over 5-10% of nights inherits the MTX night drift while
    # shrinking beta under |beta| < 0.15. QNT-99 measured purely RANDOM 5-10%
    # long-night masks passing all four house gates 10-15% of the time.
    # `beta_per_w = beta / mean|exec_w|` is the directionality of the book
    # *while it is on*: it reads ~1.0 for an index bet at ANY exposure and is
    # therefore not evadable by scaling down. Report `mean_abs_w` beside it -
    # the ratio is meaningless when the book is essentially never in the market,
    # so it is NaN below `_MIN_ABS_W`.
    # `mean|exec_w|` is clipped at 1.0 in the denominator only: MTX positions are
    # bounded in [-1, +1] by house convention, so a book above that is
    # unnormalised, and a denominator > 1 would shrink the ratio - the same
    # scale-evasion run the other way. Clipping makes such a book fall back to
    # plain |beta|. The REPORTED `mean_abs_w` is unclipped, so the anomaly stays
    # visible. See cta/gates.beta_per_w.
    exec_s = exec_sig.reindex(g.index).fillna(0)
    mean_abs_w = float(exec_s.abs().mean())
    _w_den = min(mean_abs_w, 1.0)
    beta_per_w = (beta / _w_den
                  if np.isfinite(beta) and _w_den >= _MIN_ABS_W else float("nan"))
    ...
    return {
        ...
        "mean_abs_w":      round(mean_abs_w, 4),
        "beta_per_w":      round(beta_per_w, 3) if np.isfinite(beta_per_w) else float("nan"),
